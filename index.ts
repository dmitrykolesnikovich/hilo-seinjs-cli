...
import {TEMPLATES, ENGINES} from './config';
import * as cmdNew from './commands/new';
import * as cmdMerge from './commands/merge';
...
const {argv} = Yargs
  ...
  .command(
    ...
    yargs => yargs
      ...
      .alias('t', 'template')
      .describe('t', `指定一个模板，不指定将进入选择模式，可选${TEMPLATES.join(', ')}。`)
      .alias('r', 'root')
      .describe('r', '指定要创建工程的目录。')
      .alias('c', 'container')
      .describe('t', `指定容器引擎，可选${ENGINES.join(', ')}`)
  )
  .command(
    'merge',
    '在现有项目中追加Sein.js的功能，但需要自己做出些许修改。',
    yargs => yargs
  ...
  .command(
    'compress',
    '（预留）压缩指定gltf或指定目录下的所有gltf资源'
  )
  .help('h')
  .alias('h', 'help');

if (['new', 'merge'].indexOf(argv._[0]) < 0) {
  showError('无此指令, 使用 `seinjs -h` 查看所有有效指令。');
}
